...
async function run()
{
  try
  {
    ...
    with_openrtm2 = core.getBooleanInput("with-openrtm2")
    ...
    if(latest_cmake)
    {
      latest_cmake_cmd = "curl -1sLf https://apt.kitware.com/keys/kitware-archive-latest.asc 2>/dev/null | gpg --dearmor - | tee /usr/share/keyrings/kitware-archive-keyring.gpg >/dev/null \\&\\& echo 'deb [signed-by=/usr/share/keyrings/kitware-archive-keyring.gpg] https://apt.kitware.com/ubuntu/ " + dist + " main' | tee /etc/apt/sources.list.d/kitware.list >/dev/null";
    }
